...
public class CarouselPagerAdapter extends FragmentPagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    public Fragment getItem(int position) {
        ...
        try {
            if (position == FragmentHome.FIRST_PAGE)
                scale = BIG_SCALE;
            else
                scale = SMALL_SCALE;

            position = position % FragmentHome.count;
    ...
    public int getCount() {
        int count = 0;
        try {
            count = FragmentHome.count * FragmentHome.LOOPS;
        } catch (Exception e) {
    ...
    private String getFragmentTag(int position) {
        return "android:switcher:" +FragmentHome.pager.getId() + ":" + position;
    }
}